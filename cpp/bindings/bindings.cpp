...
PYBIND11_MODULE(_cpp_ext, m) {
    ...
    py::enum_<Letter>(m, "Letter")
        .value("A",  Letter::A)  .value("B",  Letter::B)  .value("C",  Letter::C)
        .value("D",  Letter::D)  .value("E",  Letter::E)  .value("F",  Letter::F)
        .value("G",  Letter::G)  .value("H",  Letter::H)  .value("I",  Letter::I)
        .value("J",  Letter::J)  .value("K",  Letter::K)  .value("L",  Letter::L)
        .value("M",  Letter::M)  .value("N",  Letter::N)  .value("O",  Letter::O)
        .value("P",  Letter::P)  .value("Q",  Letter::Q)  .value("R",  Letter::R)
        .value("S",  Letter::S)  .value("T",  Letter::T)  .value("U",  Letter::U)
        .value("V",  Letter::V)  .value("W",  Letter::W)  .value("X",  Letter::X)
        .value("Y",  Letter::Y)  .value("Z",  Letter::Z)
        .value("BLANK", Letter::BLANK)
        .export_values()
        .def("__str__", [](Letter l) {
            return std::string(1, letter_to_char(l));
        })
        .def("__repr__", [](Letter l) {
            return std::string("Letter.") + letter_to_char(l);
        });
    ...
    py::enum_<SquareType>(m, "SquareType")
        ...
        .def("__str__", [](SquareType t) -> std::string {
            switch (t) {
            case SquareType::DEFAULT:       return ".";
    ...
    // ── State ──────────────────────────────────────────────────────────────────
    // Bind with shared_ptr holder so Gaddag.root (a shared_ptr<State>) works seamlessly.
    py::class_<State, std::shared_ptr<State>>(m, "State")
        ...
        .def("add_ending_arc",
             [](State& self, const std::string& letter, const std::string& ending_letter) {
                 if (ending_letter.empty()) throw std::invalid_argument("ending_letter must not be empty");
                 return self.add_ending_arc(letter, ending_letter[0]);
             },
             py::arg("letter"), py::arg("ending_letter"))
        .def("add_ending_letter",
    ...
    py::class_<Arc>(m, "Arc")
        .def(py::init<>())
        .def(py::init([](py::object /*letters_that_make_a_word*/, py::object dest) {
            if (dest.is_none()) return Arc{};
            return Arc(dest.cast<std::shared_ptr<State>>());
        }), py::arg("letters_that_make_a_word") = py::none(), py::arg("destination_state") = py::none())
    ...
    py::class_<Board>(m, "Board")
        ...
        .def_property_readonly("board", [](const Board& b) {
            py::list rows;
        ...
        .def_property_readonly("square_types", [](const Board& b) {
            py::dict d;
            for (int r = 0; r < BOARD_ROWS; r++) {
                for (int c = 0; c < BOARD_COLS; c++) {
        ...
        .def_property_readonly("_anchor_points", [](const Board& b) {
            py::set s;
        ...
        .def_property_readonly("_horizontal_cross_checks", [](const Board& b) {
            py::list rows;
            for (int r = 0; r < BOARD_ROWS; r++) {
                py::list row;
                for (int c = 0; c < BOARD_COLS; c++) {
                    py::set s;
                    for (Letter l : b.horizontal_cross_checks[r][c]) {
                        s.add(l);
                    }
                    row.append(s);
                }
        ...
        .def_property_readonly("_vertical_cross_checks", [](const Board& b) {
            py::list rows;
            for (int r = 0; r < BOARD_ROWS; r++) {
                py::list row;
                for (int c = 0; c < BOARD_COLS; c++) {
                    py::set s;
                    for (Letter l : b.vertical_cross_checks[r][c]) {
                        s.add(l);
                    }
                    row.append(s);
                }

...
TEST_CASE("Board has 225 square types", "[board]") {
  ...
  int count = 0;
  for (int r = 0; r < BOARD_ROWS; r++)
    for (int c = 0; c < BOARD_COLS; c++) count++;
  CHECK(count == 225);
}
...
TEST_CASE("Board anchor points after horizontal word", "[board]") {
  ...
  std::vector<std::pair<int, int>> expected = {{6, 7}, {8, 7}, {6, 8}, {8, 8},
                                               {6, 9}, {8, 9}, {7, 6}, {7, 10}};
...
TEST_CASE("Board anchor points after vertical word", "[board]") {
  ...
  std::vector<std::pair<int, int>> expected = {{6, 7}, {10, 7}, {7, 6}, {8, 6},
                                               {9, 6}, {7, 8},  {8, 8}, {9, 8}};
  for (const auto& [r, c] : expected) {
    CHECK(b.anchor_points[r][c]);
  }
...
TEST_CASE("Board place_word throws on out of bounds", "[board]") {
  ...
  // Now try to overflow
  CHECK_THROWS_AS(b.place_word({Letter::A, Letter::B, Letter::C}, Point{7, 14}, Point{7, 16}),
...
TEST_CASE("Scoring: RATE horizontal, DWS at center", "[scoring]") {
  auto g = make_gaddag({});
  Board b(g);
  int score = b.place_word({Letter::R, Letter::A, Letter::T, Letter::E}, Point{7, 5}, Point{7, 8});
  CHECK(score == 8);
}
...
TEST_CASE("Scoring: TRACE horizontal, DLS + DWS at center", "[scoring]") {
  auto g = make_gaddag({});
  Board b(g);
  int score = b.place_word({Letter::T, Letter::R, Letter::A, Letter::C, Letter::E}, Point{7, 3},
                           Point{7, 7});
  CHECK(score == 16);
}
...
TEST_CASE("Scoring: MIA vertical, TLS + pre-placed tile skip", "[scoring]") {
  ...
  int score = b.place_word({Letter::M, Letter::I, Letter::A}, Point{5, 5}, Point{7, 5});
  CHECK(score == 10);
}
...
TEST_CASE("Scoring: YOGA vertical, DWS + pre-placed tile skip", "[scoring]") {
  ...
  int score = b.place_word({Letter::Y, Letter::O, Letter::G, Letter::A}, Point{4, 4}, Point{7, 4});
  CHECK(score == 14);
}
...
TEST_CASE("Scoring: MIGRANT horizontal, TWS + DLS + bingo", "[scoring]") {
  ...
  Board b(g);
  int score =
      b.place_word({Letter::M, Letter::I, Letter::G, Letter::R, Letter::A, Letter::N, Letter::T},
                   Point{7, 0}, Point{7, 6});
  CHECK(score == 83);
}
...
TEST_CASE("Scoring: DOUBLES horizontal, two DWS stack to x4 + bingo", "[scoring]") {
  ...
  int score =
      b.place_word({Letter::D, Letter::O, Letter::U, Letter::B, Letter::L, Letter::E, Letter::S},
...
TEST_CASE("Scoring: STOPPED vertical, TWS + DLS + bingo", "[scoring]") {
  ...
  b.place_word({Letter::A}, Point{7, 7}, Point{7, 7});  // setup: A at center (DWS)
  int score =
      b.place_word({Letter::S, Letter::T, Letter::O, Letter::P, Letter::P, Letter::E, Letter::D},
                   Point{0, 7}, Point{6, 7});
  CHECK(score == 95);
}
...
TEST_CASE("Scoring: MIGRATE horizontal, DWS bingo", "[scoring]") {
  ...
  Board b(g);
  int score =
      b.place_word({Letter::M, Letter::I, Letter::G, Letter::R, Letter::A, Letter::T, Letter::E},
...
TEST_CASE("Scoring: ACES horizontal, overlap scores only new tile", "[scoring]") {
  ...
  int score = b.place_word({Letter::A, Letter::C, Letter::E, Letter::S}, Point{7, 5}, Point{7, 8});
  CHECK(score == 1);
}
...
TEST_CASE("Scoring: SO horizontal, cross-word below from vertical tiles", "[scoring]") {
  ...
  int score = b.place_word({Letter::S, Letter::O}, Point{5, 6}, Point{5, 7});
  CHECK(score == 5);
}
...
TEST_CASE("Scoring: BA vertical, cross-word on right side", "[scoring]") {
  ...
  int score = b.place_word({Letter::B, Letter::A}, Point{5, 7}, Point{6, 7});
  CHECK(score == 10);
}
...
TEST_CASE("Scoring: BA vertical, cross-word on left side", "[scoring]") {
  ...
  int score = b.place_word({Letter::B, Letter::A}, Point{6, 7}, Point{7, 7});
  CHECK(score == 10);
}

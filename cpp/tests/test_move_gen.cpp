...
// vert: true = vertical
bool move_exists(const std::vector<Move>& moves, const std::string& word, int sr, int sc,
                 bool vert) {
...
void verify_all_moves(const Board& original, const std::vector<Move>& moves) {
  for (const auto& m : moves) {
    ...
    CHECK(invalid.empty());
    int expected_score = original.calculate_score(m.letters, m.start.row, m.start.col, vert);

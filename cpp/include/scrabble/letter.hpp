...
inline std::string letter_to_key(Letter l) { return std::string(1, letter_to_char(l)); }

constexpr int get_letter_score(Letter l) noexcept { return LETTER_SCORES[static_cast<uint8_t>(l)]; }
...
struct std::hash<scrabble::Letter> {
  size_t operator()(scrabble::Letter l) const noexcept { return static_cast<size_t>(l); }
};

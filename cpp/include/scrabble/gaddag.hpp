...
public:
    ...
    // Add an arc for `letter`. If arc doesn't exist, creates it pointing to `dest`
    // (or a new State if dest is null). Always returns the arc's destination state.
...
public:
    ...
    // Build GADDAG from a wordlist file. Tries binary cache first when use_cache=true.
    // Cache is stored in temp/gaddag_<stem>.bin.
